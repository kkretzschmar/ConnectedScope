...
var ANIMDISTANCE = 0.035,  // Rotation animation threshold, ~2deg in radians
    ...
    ANIMINTERVAL_Z = 150; // Zoom duration scale in ms
...
Celestial.display = function(config) {
  ...
  zoom = d3.geo.zoom().projection(prjMap).center([width/2, height/2]).scaleExtent([scale, scale*15]).on("zoom.redraw", redraw);

    var canvas = d3.selectAll("canvas");
    if (canvas) canvas = d3.select(par).append("canvas");
  ...
  function load() {
    ...
    //Frames
    d3.json("/latestFramesData", function(error, json) {
      if (error) return console.warn(error);
	var delx = 0.68 / 2;
	var dely = 0.5 / 2;
	...
	var frameJson = {
	    ...
	    "features" : [
		{
		    ...
		    "geometry" : {
			"type" : "Polygon",
			"coordinates" : [[[frameCoord[0]-delx,frameCoord[1]-dely],[frameCoord[0]-delx,frameCoord[1]+dely],[frameCoord[0]+delx,frameCoord[1]+dely],[frameCoord[0]+delx,frameCoord[1]-dely],[frameCoord[0]/15-delx,frameCoord[1]-dely]]]    
		    }
		}
	    ]
	};
      
      var fram = getData(frameJson, trans);

      container.selectAll(".frames")
         .data(frameJson.features)
         .enter().append("path")
         .attr("class", "frames");
      redraw();
    });
  ...
  function resize(set) {
    ...
    scale = proj.scale * width/1024;
    canvas.attr("width", width).attr("height", height);
    zoom.scaleExtent([scale, scale*15]).scale(scale);
  ...
  function reproject(config) {
    ...
    d3.select({}).transition().duration(interval).tween("projection", function () {
      ...
      map.projection(prjMap);
      setClip(proj.clip); 
      zoom.projection(prjMap).scaleExtent([scale, scale*15]).scale(scale);
  ...
  function redraw() {  
    ...
    if (cfg.frames.show) {
     container.selectAll(".frames").each(function(d) { setStyle(cfg.frames.linestyle); map(d); context.stroke(); });
     drawOutline(true);
    }
...
var settings = { 
  ...
  lines: {
   ...
   //mars: { show: false, stroke:"#cc0000", width:1.3, opacity:.7 }
    ...
    frames: {
	show: true,
	linestyle: { stroke: "#990000", width: 1.5, opacity: 0.6 },
  }, 
  background: { 
    fill: "#000000", 
    opacity: 1, 
